...
\begin{itemize}[leftmargin=1.5em,itemsep=0.1em,topsep=0.2em,parsep=0pt]
    \item to reproduce the core PaDIS result on a different public CT dataset using a physically specified fan-beam operator;
    \item to compare the paper-described and released-code-compatible reproduction tracks, and evaluate the operator-normalised LION-physics extension introduced here; and
    \item to quantify the effects of patch size, training-set size, positional encoding, initialisation and sampler choice.
\end{itemize}
...
This work therefore combines reproduction with a controlled extension, testing the central claim across three reconstruction tracks rather than seeking numerical identity under different data and geometry. The remaining chapters present the background, methods, results and discussion before the conclusion.

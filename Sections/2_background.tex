...
This chapter introduces only the concepts needed to interpret the experiments. These are the CT forward problem and its missing-data regimes, variance-exploding diffusion priors, the PaDIS patch construction and the reconstruction algorithms used for comparison.

\section{Computed Tomography}
\label{sec:basics}

X-ray computed tomography (CT) reconstructs a map of linear attenuation from transmission measurements acquired around an object \cite{}. For a ray path \(L\), the Beer--Lambert model gives
\begin{equation}
...
Parallel-beam CT uses parallel rays at each view and is the geometry used for the main experiments of Hu et al. \cite{}. Fan-beam rays instead diverge from a finite source towards a line detector. This is closer to clinical acquisition and is the geometry used here \cite{}.

Each parallel-beam view samples a radial line of the object's Fourier transform. Fan-beam sampling differs, but the same intuition applies. Fewer views leave angular gaps and produce streak artefacts, while limited-angle acquisition omits a complete sector and creates direction-dependent ambiguity \cite{}. Learned priors can fill these gaps, but may also add plausible structure unsupported by the measurements. Image quality must therefore be considered alongside measurement consistency.

\subsection{Dose, Noise and Display}
\label{subsec:ct_noise_display}

Dose can be reduced using fewer views or photons, but these create different inverse problems. Fewer views increase angular undersampling, whereas low photon counts increase signal-dependent noise. Photon detection is approximately Poisson \cite{},
\begin{equation}
C\sim\operatorname{Poisson}\!\left(C_0e^{-P}\right),
\label{eq:poisson_photon_count}
\end{equation}
so log-transformed projections become noisy and biased at low counts. Like Hu et al. \cite{}, we use simulated noiseless sinograms and therefore test missing angular information rather than realistic low-photon-dose performance.

CT values are commonly displayed in Hounsfield units (HU) \cite{},
\begin{equation}
...
Diffusion models learn to reverse a gradual noise process \cite{}. Score-based variants estimate the noisy log-density gradient needed to denoise or sample.
...
The choices of $\mathbf{f}$ and $g$ in \eqref{eq:general_sde} define different families of SDEs \cite{}. In a variance-preserving (VP) SDE, the signal is attenuated while noise is added according to
...
Therefore, using Tweedie's formula \cite{} gives
...
We can therefore interpret a VE-denoised prediction, $D_{\boldsymbol{\phi}}(\mathbf{z},\sigma)$, as a clean-image estimate, enabling direct application of medical imaging measurement models. For example, in CT reconstruction, we can predict the corresponding sinogram as $AD_{\boldsymbol{\phi}}(\mathbf{z},\sigma)$, where $A$ is the CT forward operator \cite{SongMedicalInverse, ChungYeMRI, DPS}.

\subsection{EDM Preconditioning} \label{subsec:edm_preconditioning}

In some cases, we choose not to use a denoising model directly, but instead embed one within a preconditioning framework. This is the case for EDM \cite{Karras_edm}, which wraps a raw neural network, $F_{\boldsymbol{\phi}}$, with scale-dependent input, output and skip coefficients to form a preconditioned denoiser. This gives the raw network a better-conditioned learning problem because inputs have a similar scale across noise levels, and the network only needs to predict the part of the denoising map not already captured by the skip connection. For a noisy image $\mathbf{z}$ at noise level $\sigma$, this gives
...
Using this, Hu et al. define a patch denoising objective \cite{},
...
The implementation fixes the padded border at zero and omits its score, while the patch terms are learned by the denoising network. Computing this over all possible partitions at each sampling step would be prohibitively expensive, so sampling and reconstruction instead select one random shift at each iteration and use a stochastic estimate of the whole-image score. For a selected shift $(i,j)$, this gives
...
Filtered back-projection (FBP) reconstructs parallel-beam data by applying a ramp filter to each projection before back-projecting it through the image. The filter compensates for the over-representation of low spatial frequencies, making FBP fast and interpretable when angular sampling is dense. Sparse views instead produce characteristic streaks because the missing angular information cannot be recovered by filtering alone \cite{}.

Fan-beam and cone-beam analogues additionally weight projections for the divergent ray geometry \cite{}. LION uses its FDK implementation as the analytic baseline for the fan-beam experiments in this work.
\subsection{Iterative Reconstruction and Denoising Priors}
\label{subsec:iterative_reconstruction_denoising_priors}

Instead of trying to perform reconstruction in one analytical operation, iterative reconstructions treat reconstruction as a data-fitting problem, repeatedly updating an initial image estimate to make its forward projection better match the measured sinogram \cite{}. For a forward model
...
Often, a total-variation regulariser is used \cite{}, where
...
Chambolle--Pock (CP) solves the non-smooth isotropic TV objective with alternating primal and dual updates \cite{}. Its proximal steps handle TV directly and avoid ADMM's geometry-dependent linear solve, so the same formulation applies to different CT operators.

Instead of these explicit regularisers, plug-and-play methods use a learned denoiser within an iterative algorithm \cite{}. The PnP-ADMM method used here separates the data-fitting image, $\mathbf{x}$, denoised image, $\mathbf{v}$, and scaled dual variable, $\mathbf{u}$, with the data update
...
As discussed in \secref{sec:score_based_sdes}, we can use a variance exploding denoiser to estimate the score. At a fixed noise level, $\sigma_k$, this can be used to inform a Langevin update \cite{} to an existing reconstruction $\tilde{\mathbf{z}}^k$ as
...
To perform reconstruction, we can combine this prior update with a data-consistency update by inserting a gradient step against the measurement residual,
...
Diffusion posterior sampling (DPS) instead applies the sinogram measurement model to the denoised estimate \cite{}, such that if we define the clean image as
...
which we can then again correct towards the regions of higher probability under the learned image prior using the same Langevin step from \eqref{eq:Langevin_step}. This is known as the predictor-corrector (PC) method \cite{}.

Instead of descending the square residual, DDNM methods \cite{} impose measurement consistency through an approximate pseudoinverse of the form

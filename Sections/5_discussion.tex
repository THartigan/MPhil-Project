...
We were able to reproduce several of the main findings. Hu et al. reported PaDIS PSNRs of \(33.57\,\mathrm{dB}\) at 20 views and \(29.48\,\mathrm{dB}\) at 8 views \cite{}. Our paper-style track reached \(32.71\) and \(27.15\,\mathrm{dB}\). The 20-view result is therefore within \(0.86\,\mathrm{dB}\), despite the changed data and geometry, whilst the 8-view result still improves substantially over FDK and our CP-TV baseline. We also found that PaDIS can generate coherent whole CT slices after training only on patches. In comparison, fixed stitching and averaging produced much less credible global anatomy, supporting the use of randomly shifted partitions.

We also reproduced the practical high-resolution argument \cite{}. We trained and applied a \(512\times512\) PaDIS prior without training a whole-image model at this resolution, obtaining \(40.08\,\mathrm{dB}\) PSNR and \(0.960\) SSIM. At 256 resolution, PaDIS-DPS reconstructed slices \(24\%\) faster than whole-image VE-DPS, after half the training time. PaDIS is therefore still useful where whole-image memory or training cost is prohibitive.
...
However, this does not mean that the whole-image prior is always better. Within the LION-physics extension, PaDIS with VE-DDNM achieved the best 20- and 60-view PSNRs, and beat whole-image VE-DDNM by \(2.30\,\mathrm{dB}\) at 20 views. Langevin and VE-DPS favoured the whole-image model, whilst predictor-corrector changed the ranking depending on the metric. For this reason, our results suggest that PaDIS performance depends on how its patch score is used by the sampler. We can reproduce the claim that PaDIS is a flexible and sometimes stronger prior, but not that it always outperforms whole-image diffusion.
...
The patch-size study does not show a simple benefit from smaller patches. The \(96\times96\) model matched whole-image VE-DPS within uncertainty, whilst the default \(56\times56\) model was \(1.43\,\mathrm{dB}\) lower. The non-monotonic improvement at \(8\times8\) suggests that receptive field, optimisation, number of patches and partition geometry all contribute. Nevertheless, \autoref{tab:patch-size} and \autoref{fig:results-patch-size} show that retaining more global context is beneficial when memory permits. At this image size, the clearest PaDIS advantage is feasibility, rather than improved quality from very small patches.
...
Removing position channels also caused almost no reconstruction loss, unlike the failure reported by Hu et al. \cite{}. The CT measurements and repeated data-consistency updates appear to provide enough global information to locate local anatomy. This does not show that position is unnecessary for unconditional generation, where no measurements provide location. Similarly, FDK and Gaussian initialisation converged to almost identical results after 1000 model evaluations. The geometric noise schedule was more important, improving PSNR by about \(1.3\,\mathrm{dB}\) over the public EDM-style default.

\subsection{Study Specification and Implementation}
\label{sec:discussion-reproducibility}

We treated the released code as part of the original study's specification, rather than as a separate implementation \cite{}. Even together, the paper and code leave several choices ambiguous. The variable-size U-Net is attributed to different references, whilst the number of sampled patches, validation split, checkpoint rule and 25 test slices are not given. View and detector counts are reported without the physical field of view, detector spacing or complete angular conventions needed to define \(A\). The released 180-view fan-beam configuration also spans only \(180^\circ\), which is unclear from the main description. Fine-tuning is reported without the search or selection protocol. There are smaller reporting inconsistencies. Table 1 does not always bold the best SSIM, and Table 7 is captioned as a dataset-size study despite comparing samplers.

The code resolves some details, but differs from the written algorithms \cite{}. Its main DPS path starts from filtered back-projection rather than the Gaussian state in Algorithm 1, differentiates the residual norm rather than its square, and defaults to an EDM-style schedule. Predictor-corrector changes the corrector noise level and reuses its patch layout. VE-DDNM uses 100 noise levels with 10 inner updates rather than 1000 separate levels. We also restored several comparison APIs before the published methods could run. These ambiguities within the combined specification can materially change the reconstruction.

Comparison with the adapted public-code fork showed agreement within sampler variation on the same LION geometry. This validates the Public-compatible implementation, but not the paper's unspecified ODL geometry. Publishing geometry, data indices, configurations and checkpoints would reduce this ambiguity.

\section{Extensions to the Original Work}

Paper-style and Public-compatible are two readings of the combined specification. LION-physics is an extension introduced here, replacing geometry-specific calibration with an operator-derived scale. Its results assess transfer and stabilisation, not direct reproduction of Hu et al.'s algorithms.
...
As in the original CT experiments \cite{}, our sinograms omit photon noise, scatter, beam hardening and motion. We therefore study angular undersampling, not realistic low-dose CT. The experiments are two-dimensional, use one dataset and training run per model, and evaluate 25 slices rather than independent scans. Fixed-overlap and native-resolution rows use four. Bootstrap intervals measure variation across slices, not retraining uncertainty, and some high-view methods inherit 20-view tuning. PSNR, SSIM and MAE also cannot determine whether a nodule was preserved or hallucinated.
